...
public class UnbanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        player.thenAccept(punishPlayer -> {
            ...
            if (!PunishmentUtil.hasActivePunishment(punishPlayer, Punishment.PunishmentType.BAN)) {
                sender.sendMessage("Player is not banned!");
                return;
            }
            Punishment punishment = PunishmentUtil.getActivePunishment(punishPlayer, Punishment.PunishmentType.BAN);
            punishment.setExpired(true);
            plugin.getDatabase().savePunishment(punishment);
            String message = StringSubstitutor.replace(plugin.getMessagesConfig().getString("commands.ban.unban_broadcast"),
